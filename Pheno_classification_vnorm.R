...
# Read data
################################################################################
huh_spec_vnorm  = read.csv("FloraPalooza2026/Data/DMWhiteHUHspec1_sp25leaf560_resampled_5nm_norm_updated.csv")

#######################################
# Process data
#######################################
# Convert "day of collection" to months
huh_spec_vnorm$month = as.numeric(format(
  as.Date(huh_spec_vnorm$doyOfCollection - 1, origin = "2025-01-01"),
  "%m"
))
...
huh_spec_vnorm$Phenophase  = NA
huh_spec_vnorm = huh_spec_vnorm %>%
  mutate(
...
huh_spec_vnorm = huh_spec_vnorm %>%
  dplyr::select(
    filename, simpleFilename, projectID, normalization_magnitude, opticalSetupDescription, lightSourceType, 
...
train_idx = createDataPartition(huh_spec_vnorm$Phenophase, p = 0.8, list = FALSE)

data_train = huh_spec_vnorm[train_idx, ]
data_test  = huh_spec_vnorm[-train_idx, ]

# Variables for prediction
predictor_columns = 55:ncol(huh_spec_vnorm)
...
table(huh_spec_vnorm$Phenophase)
...
# Save
saveRDS(cm_pls, "FloraPalooza2026/Data/PLSDA_confusion_matrix_huh_herbarium_Phenophase_vnorm.rds")
...
# Save
saveRDS(pls_coef, "FloraPalooza2026/Data/PLSDA_coefficients_vnorm.rds")
...
plsda_plot = ggplot(diag_percent, aes(x = Prediction, y = Reference, fill = Percent)) +
  ...
  labs(x = "", # "Predicted Identity",
       y = "True Identity", 
       title = "HU Herbarium Spectra (vnorm) - White et al. 2025",
       subtitle = "PLS-DA") +
...
plsda_vip_plot = ggplot(pls_vip_df, aes(x = Wavelength, y = VIP)) +
  ...
  labs(x = "Wavelength (nm)",
       y = "VIP Score",
       title = "PLS-DA Variable Importance (VIP) - vnorm",
...
ggsave("FloraPalooza2026/Figs/Phenophase_herbarium_vnorm_VIP_PLSDA_plot.pdf", plot = plsda_vip_plot, height = 6, width = 8.5, dpi = 600)
ggsave("FloraPalooza2026/Figs/Phenophase_herbarium_vnorm_VIP_PLSDA_plot.png", plot = plsda_vip_plot, height = 6, width = 8.5, dpi = 600)
...
# Save
saveRDS(cm_lda, "FloraPalooza2026/Data/LDA_confusion_matrix_HUH_herbarium_vnorm_Phenophase.rds")
...
# Save
saveRDS(lda_coef, "FloraPalooza2026/Data/LDA_coefficients_vnorm.rds")
...
ggsave("FloraPalooza2026/Figs/Phenophase_Herbarium_vnorm_VI_LDA_plot.pdf", plot = lda_vi_plot, height = 6, width = 8.5, dpi = 600)
ggsave("FloraPalooza2026/Figs/Phenophase_Herbarium_vnorm_VI_LDA_plot.png", plot = lda_vi_plot, height = 6, width = 8.5, dpi = 600)
...
ggsave("FloraPalooza2026/Figs/Phenophase_Classification_HUHerbarium_vnorm_Spectra.pdf", plot = class_models,
       height = 8.5, width = 7, dpi = 600)
ggsave("FloraPalooza2026/Figs/Phenophase_Classification_HUHerbarium_vnorm_Spectra.png", plot = class_models,
       height = 8.5, width = 7, dpi = 600)
...
save_classification_model_results = function(model, cm, filename_prefix) {
  ...
  saveRDS(results_list,
          paste0("FloraPalooza2026/Data/", filename_prefix, "_full_results_herbarium_vnorm_phenophase.rds"))
}
...
write.csv(all_models_summary, 
          "FloraPalooza2026/Data/Pheno_Classification_Model_Validation_Summary_Herbarium_vnorm_Phenophase.csv",
          row.names = FALSE)
...
write.csv(herbarium_model_comparison, 
          "FloraPalooza2026/Data/Phenophase_Herbarium_classification_models_vnorm_comparison.csv", 
          row.names = F)
